...
def non_empty_group_list(db, app):
    if len(db.get_group_list()) == 0:
        app.group.create(Group(name="new name to delete"))
    return db.get_group_list()
...
@then("the new group list is equal to the old group without the deleted group")
...
def modify_group(app, random_group):
    app.group.modify_group_by_id(random_group.id, Group(name="modify me"))

@then("the new group list is equal to the old group with modified group")
def verify_group_modified(db, non_empty_group_list, check_ui):
    old_groups = non_empty_group_list
    new_groups = db.get_group_list()
